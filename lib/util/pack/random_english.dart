...
我在进行英文学习，具体是我会根据1个中文文本和4个英文文本，选择最正确的英文文本
我会给你一个中文的文本和对应的英文的文本，我需要你参考这两个文本，生成剩下有迷惑性3个答案：

注意：如果给的是一个单词，返回的迷惑项目也要是单词，如果给的是一个短语/句子，返回的是单词量相近的短语/句子
注意：返回的迷惑项目与正确答案不能意思一样，要有区分度
...
Map<String, dynamic> parseRes(String res) {
  try {
    ...
    if (start == -1 || end == -1 || start >= end) {
      throw FormatException('未找到有效的JSON代码块');
    }
    
    // 提取```之间的内容
    String jsonStr = res.substring(start + 3, end).trim();
    
    // 移除可能的语言标识符（如json、JSON等）
    ...
    // 查找第一个{和最后一个}，确保提取完整的JSON对象
    final jsonStart = jsonStr.indexOf('{');
    final jsonEnd = jsonStr.lastIndexOf('}');
    
    if (jsonStart == -1 || jsonEnd == -1 || jsonStart >= jsonEnd) {
      throw FormatException('未找到有效的JSON对象');
    }
    
    jsonStr = jsonStr.substring(jsonStart, jsonEnd + 1);
    
    final resObj = jsonDecode(jsonStr);
    return resObj;
  } catch (e) {
    print('JSON解析错误: $e');
    print('原始响应: $res');
    // 返回默认值，避免应用崩溃
    return {'result': ['Option1', 'Option2', 'Option3']};
  }
}

Future<List<String>> doRandomEnglish(String cn, String en) async {
